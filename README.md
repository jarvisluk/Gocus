...
Gocus is a compact macOS Git companion that sits beside your editor and keeps the current repository, branch, worktrees, changed files, and recent commits visible without opening a full Git client.
...
The footer changed-files button shows how many files changed right now. Open it to inspect the changed-file list, then open the repository or a file in an enabled external app such as Codex, VS Code, Finder, Terminal, Cursor, or Xcode.
