...
export function SettingsPanel({
  ...
  const [settingsPage, setSettingsPage] = useState<"main" | "openIn">("main");
  ...
  const enabledWorkspaceOptionCount = availableWorkspaceOptions.filter((option) => preferences.workspaceOpenTargets.includes(option.target)).length;
  const workspaceTargetsSummary = availableWorkspaceOptions.length ? `${enabledWorkspaceOptionCount} enabled` : "Unavailable";
  const openInPageActive = settingsPage === "openIn";

  useEffect(() => {
    if (!openInPageActive) return undefined;
  ...
  return (
    <section className="ui-page settings-page" aria-label={openInPageActive ? "Open in settings" : "Settings"}>
      <header className="ui-page-header settings-page-header">
        <button className="ui-icon-button" type="button" aria-label={openInPageActive ? "Back to settings" : "Back to Git view"} onClick={handleBack}>
          <ChevronLeft aria-hidden="true" />
        </button>
        <div>
          <h1>{openInPageActive ? "Open in" : "Settings"}</h1>
          <span>{openInPageActive ? "External apps" : "Interface preferences"}</span>
        </div>
      </header>

      {openInPageActive ? (
        <div className="ui-form settings-panel settings-workspace-targets-panel">
          {availableWorkspaceOptions.length ? (
            ...
            <div className="empty-state">No external apps available.</div>
          )}
        </div>
      ) : (
        <div className="ui-form settings-panel">
          <div className="ui-form-section" aria-labelledby="settings-appearance-title">
            <h2 className="ui-form-section-title" id="settings-appearance-title">
              Appearance
            </h2>
          ...
          <div className="ui-form-section" aria-labelledby="settings-graph-title">
            <h2 className="ui-form-section-title" id="settings-graph-title">
              Graph
            </h2>
            <div className="ui-form-row settings-row">
              <span className="ui-label">Lines</span>
              <div className="ui-segmented segmented-control compact">
          ...
          <div className="ui-form-section" aria-labelledby="settings-behavior-title">
            <h2 className="ui-form-section-title" id="settings-behavior-title">
              Behavior
            </h2>
            <div className="ui-form-row settings-row">
            ...
            <div className="ui-form-row settings-row">
              <span className="ui-label">Zen entry</span>
          ...
          <div className="ui-form-section" aria-labelledby="settings-workspace-title">
            <h2 className="ui-form-section-title" id="settings-workspace-title">
              Workspace
            </h2>
            <div className="ui-form-row settings-row">
              <span className="ui-label">Open in</span>
              <div className="ui-select-frame ui-disclosure-frame">
                <button className="ui-disclosure-button" type="button" aria-label="Open external app settings" onClick={() => setSettingsPage("openIn")}>
                  <span className="ui-disclosure-label">Apps</span>
                  <span className="ui-disclosure-value">{workspaceTargetsSummary}</span>
          ...
          <div className="ui-form-section ui-form-section-actions" aria-label="Reset preferences">
