...
function bridgeJoinY(
  ...
  const availableDrop = Math.max(1, rowBottomY - fromY);
...
function addBridgeLine(
  ...
  const laneDelta = toX - fromX;
  const startOffset = startsAtNode ? Math.sign(laneDelta) * 3 : 0;
  const startX = fromX + startOffset;
  const joinY = bridgeJoinY(fromY, rowBottomY, fromX, toX, connectsToExistingLane);
  const drop = joinY - fromY;
  const controlDrop = Math.max(8, drop * 0.36);
  const controlFromY = fromY + controlDrop;
...
export function buildGitTreeCanvasModel({
  ...
  commits.forEach((commit, visibleIndex) => {
    ...
    if (graph.currentContinues) {
      addVerticalLine(lines, graph.column, rowTopY, nodeY, graph.incomingColor, graph.incomingVariant);
    }

    graph.parentStems.forEach((lane) => {
      addVerticalLine(lines, lane.column, nodeY, rowBottomY, lane.color, lane.variant);
    });
