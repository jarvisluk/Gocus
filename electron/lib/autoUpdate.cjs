...
function updateRepositoryFromPackage(packageMetadata = {}) {
  return normalizeUpdateRepository(
    process.env.GIT_PEEK_UPDATE_REPO ||
...
function buildUpdateFeedUrl({
  ...
  arch = process.arch,
  version,
  server = process.env.GIT_PEEK_UPDATE_SERVER || defaultUpdateServer,
} = {}) {
...
function createAutoUpdateController({
  ...
  function initialize() {
    ...
    autoUpdater.on("update-available", () => {
      logger.info("[Git Peek] Update available.");
    });
    autoUpdater.on("update-not-available", () => {
      checking = false;
      if (manualCheckPending) {
        showManualMessage("Git Peek is up to date.", `Current version: ${app.getVersion()}`);
      }
    ...
    autoUpdater.on("update-downloaded", (_event, releaseNotes, releaseName) => {
      ...
      dialog
        .showMessageBox({
          ...
          detail: releaseNotes ? String(releaseNotes) : "Git Peek will relaunch after installing the update.",
        })
        .then((result) => {
        ...
        .catch((error) => logger.warn("[Git Peek] Unable to show update install prompt.", error));
      manualCheckPending = false;
    });
    autoUpdater.on("error", (error) => {
      ...
      if (manualCheckPending) {
        ...
        logger.warn("[Git Peek] Auto-update check failed.", error);
      }
      manualCheckPending = false;
    });
  ...
  function checkForUpdates({ manual = false } = {}) {
    if (!initialize()) {
      if (manual) {
        showManualMessage(
          ...
          "Git Peek can check GitHub Releases only from a packaged macOS app.",
        );
      }
      return false;
    }
    ...
    if (checking) {
      if (manual) showManualMessage("Git Peek is already checking for updates.");
      return false;
    }
