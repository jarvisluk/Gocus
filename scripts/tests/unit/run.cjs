...
async function testAutoUpdateModule() {
  ...
  const previousUpdateRepo = process.env.GIT_PEEK_UPDATE_REPO;
  delete process.env.GIT_PEEK_UPDATE_REPO;
  ...
  try {
    ...
    assert.equal(dialogs.at(-1).message, "Git Peek is up to date.");

    controller.checkForUpdates({ manual: true });
    events.emit("update-downloaded", {}, "Fixed spacing", "Git Peek 0.2.1");
    await Promise.resolve();
    assert.equal(dialogs.at(-1).message, "Git Peek 0.2.1 is ready to install.");
    ...
    if (previousUpdateRepo === undefined) {
      ...
      process.env.GIT_PEEK_UPDATE_REPO = previousUpdateRepo;
    }
  }
}
