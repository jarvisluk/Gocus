...
include the macOS platform and architecture in the zip name. Windows installer
builds use `electron-updater` with GitHub Releases metadata. Windows portable
assets are downloadable release assets and must be updated manually.
...
Unsigned Windows installer updates may still trigger SmartScreen warnings.
...
Packaged macOS builds and Windows installer builds check for updates shortly
after launch and then periodically. Users can also run **Check for Updates...**
from the app menu, Help menu, or menu bar icon menu. Windows portable builds
report that automatic updates are unavailable.
...
The Windows installer feed is backed by the `latest.yml` and blockmap files
generated by `electron-builder`, so each release needs non-draft assets named
like:
...
Windows portable release assets are still published to GitHub Releases for
direct download:
